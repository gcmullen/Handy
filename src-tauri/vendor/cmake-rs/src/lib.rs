...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        // PATCH: Under Ninja, CMake infers a Visual Studio "instance" from
        // VSINSTALLDIR (injected just above via the MSVC compiler env) and errors
        // because Ninja doesn't support instance specification. Drop it; cl.exe is
        // still found via PATH/INCLUDE/LIB. Fixes whisper.cpp's vulkan-shaders-gen
        // ExternalProject build under Ninja.
        if is_ninja && msvc {
            cmd.env_remove("VSINSTALLDIR");
        }
        ...
        // PATCH: see above — drop VSINSTALLDIR under Ninja so the nested
        // ExternalProject configure (spawned during the build) doesn't infer a VS
        // instance and fail.
        if is_ninja && msvc {
            cmd.env_remove("VSINSTALLDIR");
        }
